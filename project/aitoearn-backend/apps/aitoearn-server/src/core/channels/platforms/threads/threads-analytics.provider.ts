...
@Injectable()
export class ThreadsAnalyticsProvider implements AnalyticsProvider {
  ...
  async fetchAccountAnalytics(input: AnalyticsAccountInput): Promise<ChannelAccountAnalyticsResult> {
    ...
    if (!platformUid) {
      throw ThreadsPlatformException.validation({
        ...
        context: {
          endpoint: 'fetchAccountAnalytics',
          accountId: input.accountId,
        },
      })
    }

    const response = await this.threadsService.getAccountInsights(
      platformUid,
    ...
    const metricMap: Record<string, keyof ChannelAccountMetricsSnapshot> = {
      ...
      reposts: 'forwardCount',
      quotes: 'engagementCount',
      followers_count: 'fansCount',
    }
    ...
    for (const item of data) {
      for (const value of item.values ?? []) {
        ...
        const metricKey = item.name ? metricMap[item.name] : undefined
        if (metricKey) {
          snapshot.metrics[metricKey] = Number(value.value ?? 0)
        }
        else if (item.name) {
          snapshot.extra[item.name] = value.value
        }
  ...
  async fetchWorkAnalytics(input: AnalyticsWorkInput): Promise<ChannelWorkAnalyticsResult> {
    const response = await this.threadsService.getPostInsights(
